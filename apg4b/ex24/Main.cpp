...
struct Clock {
  ...
  string to_str() {
    char c_str[] = "YY:MM:DD";
    sprintf(c_str, "%02d:%02d:%02d", hour, minute, second);

    return (string)c_str;
  }

  void shift(int diff_second) {
    int curr_second = hour * 3600 + minute * 60 + second;
    curr_second += diff_second;
    if (curr_second < 0) curr_second += 86400;
    curr_second = curr_second % 86400;

    hour = curr_second / 3600;
    minute = (curr_second % 3600) / 60;
    second = (curr_second % 3600) % 60;
  }
};
